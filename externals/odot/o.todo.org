...
** Bugs
*** General
**** BUG All objects need to handle naked (valid) OSC messages and convert them to bundles. [17/21]
- [X] o.atomize
see o.expr
- [X] o.change
no change necessary
...
**** BUG o.expr crashes if instantiated with a function with arity > 0 with no args
*** o.message
**** BUG osc_message_u_getAddress crashed
**** BUG 0   edu.cnmat.berkeley.o.route    0x0f9dfdec osc_message_s_wrap + 12
1   edu.cnmat.berkeley.o.route    	0x0f9df18c osc_bundle_iterator_s_next + 92
2   edu.cnmat.berkeley.o.route    	0x0f9e7d38 osc_query_select_impl + 120
3   edu.cnmat.berkeley.o.route    	0x0f9e80b0 osc_query_select + 112
4   edu.cnmat.berkeley.o.route    	0x0f9dc03e oroute_fullPacket + 78
*** o.prepend
**** BUG o.prepend seems to not handle regular max messages in a consistent way
This is a bug
*** o.rename
...
** Feature Requests
*** General
**** DELEGATED Overview / tutorial patches
Jeff and/or Yotam are working on this
...
*** o.message
**** TODO Post an error when a non-OSC message gets sent into the right inlet
*** o.expr
...
**** TODO In osc_expr.c, variable names like function and expr are used inapproapriately in osc_expr_funcall.
In general, expressions should be called something like expr_lex (we need a way to represent
tokenized expressions), expr_str, or expr_ir.
As it is now, things that are really compiled expressions are being called functions.
The use of ``function_graph'' in o.expr.c may also be inappropriate.
**** TODO argc(), exists(), bound() should be isbound()?
*** o.if
*** o.printbytes
