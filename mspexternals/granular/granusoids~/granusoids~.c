/*--
 ...
 adding test version with precalculated shaping lookup tables to avoid using pow() in inner loop
...
//    uint32_t    prev_phase; //maybe useful if interpolating
    double        phase_inc;
...
int main(void)
{	
	t_class *c = class_new("granusoids~", (method)grans_new, (method)grans_free, (long)sizeof(t_grans), NULL,   A_GIMME);
...
void grans_perform64(t_grans *x, t_object *dsp64, double **ins, long numins, double **outs, long numouts, long sampleframes, long flags, void *userparam)
{
    ...
    int j, i;
    const double *sinetab = x->sinetab;
    ...
    for( i = 0; i < maxoscillators; i++, o++)
    {
        ...
        if( w_type == 0 ) //cos window
        {
            while(j < sampleframes && wpc < STABSZ)
            {
                ...
                outlets[outletnum][j] += amp * sinetab[ (uint32_t)pc ];
    ...
    for ( i = 0; i < maxoscillators; ++i )
    {
        if( o->phase_inc == 0.0 )
        {
           ...
           // o->tex = 1.0 / ((attack > 1.0) ? 1.0 : attack);
            ...
            o->window_attack = CLAMP(attack, 0.0, 1.0);
            o->window_slope = CLAMP(slope, 0.0, 1.0);
