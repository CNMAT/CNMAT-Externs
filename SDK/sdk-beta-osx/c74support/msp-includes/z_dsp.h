...
typedef struct _dspchain
{
	...
	t_signal *c_inputs;		// signal input list (zero before first exec)
	t_signal *c_outputs; 	// signal output list
...
t_dspchain *dspchain_compile(struct patcher *p, long bs, long sr);
void dspchain_tick(t_dspchain *c);
void canvas_start_onedsp(struct patcher *p, t_dspchain **c, long bs, long sr); 
