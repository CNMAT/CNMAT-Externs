...
void max_jit_textfile_read(t_max_jit_textfile *x);
...
void max_jit_textfile_tobuffer(t_max_jit_textfile *x);
...
void max_jit_textfile_opentextfile_write(t_max_jit_textfile *x, t_symbol *s, long ac, t_atom *av)
{
	char filename[256] = "matrixtextfile.txt";
	short path, err;
	long type = 'TEXT';
	t_filehandle fh_write;
	
	if (av && ac) {
		strcpy(filename, av->a_w.w_sym->s_name);
		path = x->path;
	}
