...
void jslider_orientation(t_jslider *x, t_rect *r);
double jslider_postoval(t_jslider *x, t_pt pt, t_rect *r);
double jslider_valtopos(t_jslider *x, double val, t_rect *r);
...
void jslider_paint(t_jslider *x, t_object *view)
{
	...
	jslider_orientation(x, &rect);
	pos = jslider_valtopos(x, x->j_val, &rect);
	if (x->j_horiz) {
...
void jslider_orientation(t_jslider *x, t_rect *r)
{
	if (x->j_orientation == 0) { // automatic mode
		...
		x->j_horiz = x->j_orientation == 2 ? 0 : 1;	
}
...
void jslider_mousedown(t_jslider *x, t_object *patcherview, t_pt pt, long modifiers)
{
	...
	val = jslider_postoval(x,pt,&rect);
	if (!x->j_relative)
