...
{	
	...
	c = class_new("iterator", (method)iterator_new, (method)iterator_free, (long)sizeof(t_iterator), 0L, A_GIMME, 0);
...
//	post("got %s message from %X (%s)", msg->s_name, sender, s->s_name);
	if (msg == gensym("attr_modified")) { // sent when an attribute of the sender changes
		t_symbol *attrname;
...
//	t_object *patcher = gensym("#P")->s_thing;
//	t_object *box = gensym("#B")->s_thing;
    
	if (x = (t_iterator *)object_alloc(iterator_class)) {
		t_object *pv;
		
		// this will only work if the patcher is visible
		// beware!
		pv = jpatcher_get_firstview(gensym("#P")->s_thing);
		// pv happens to be a registered object
		// so we can attach to it using object_attach_byptr.
		// david mentioned object_attach_byptr_register() for
		// more advanced attachment applications.
		object_attach_byptr(x, pv); // new for Max 5		
		{
			t_atom *av = NULL;
			long ac = 0;
			
			// get the bounds of the first patcherview
			object_attr_getvalueof(pv, gensym("rect"), &ac, &av);
			if (ac && av) {
